...
static uint32_t calc_pcr_digest(uint8_t* hv_pk_buf,
  ...
  if (memcmp(hv_pub_pcr, &pcr_array[array_size - sha_len], sha_len) != 0) {
    SPDLOG_ERROR("PCR 13 does not match!");
...
static uint8_t get_pcr_array(TPML_PCR_SELECTION* pcr_select,
  ...
  if (!pcr_select || !pcr_index) {
    SPDLOG_ERROR("get_pcr_array bad parameter");
    return 0;
  }
  if (pcr_select->count <= 0) {
    SPDLOG_ERROR("Invalid PCR select count: %d", pcr_select->count);
    return 0;
  }
...
bool decode_tpm_attest_data(uint8_t* data, uint16_t size, TPMS_ATTEST* attest) {
  ...
  if (data == NULL || size <= 0 || attest == NULL) {
    SPDLOG_ERROR("decode_tpm_attest_data bad parameter");
    return false;
  }
  ptr = data;
  ...
  if (attest->tpm_generated != TPM_GENERATED_VALUE) {
    SPDLOG_ERROR("Invalid tpm generated");
    return false;
  }
  if (attest->type != TPM_ST_ATTEST_QUOTE) {
    SPDLOG_ERROR("invalid tpm attest type");
    return false;
  }
  return true;
}
...
bool verify_pcr_digest(TPMS_ATTEST* attest,
  ...
  uint8_t quote_pcr_index[MAX_PCR_NUM];
  ...
  if (quote_pcr_count <= 0 || quote_pcr_count != sizeof(expected_pcr_index)) {
    SPDLOG_ERROR("Invalid pcr index num: %d", quote_pcr_count);
    return false;
  }
  if (memcmp(expected_pcr_index, quote_pcr_index, sizeof(expected_pcr_index)) !=
      0) {
    SPDLOG_ERROR("PCR index verification failed");
    return false;
  }

  SPDLOG_INFO("TPM PCR index list verification: Success");
  //Since the open-source HyperEnclave does not support certificate mode, we skip the calc_pcr_digest step
  return true;
}
...
static int parse_der_cert(uint8_t* cert,
  ...
  if (len <= 0 || !ptr) {
    SPDLOG_ERROR("Fail to parse certificate");
    return 0;
  }
  uint8_t* head = ptr;
  ...
  if (*ptbs_len < len) {
    SPDLOG_ERROR("Too small tbs_cert buff: %d<%d", *ptbs_len, len);
    return 0;
  }
  ...
  *ptbs_len = len;
  ...
  if (*psig_len < 2 * SM2_COOR_SIZE) {
    SPDLOG_ERROR("Invalid signature buffer");
    return 0;
  }
...
static int sm2_compute_z_digest(uint8_t* out,
  ...
  if (id_len >= (UINT16_MAX / 8)) {
    SPDLOG_ERROR("Too large id length");
    return 0;
  }
...
static int crypto_sm2_verify(uint8_t* msg,
  ...
  if (!ec_key || EC_KEY_set_group(ec_key, g.get()) <= 0) {
    SPDLOG_ERROR("EC_KEY_set_group err");
    return 0;
  }
  int ret = EC_KEY_set_public_key(ec_key, p.get());
  if (ret != 1) {
    SPDLOG_ERROR("EC_KEY_set_public_key ret=%d", ret);
    return 0;
  }
  ...
  if (ecdsa_sig.get() == nullptr) {
    SPDLOG_ERROR("ecdsa_sig is null");
    return 0;
  }
  ret = sm2_do_verify_sig(ec_key, EVP_sm3(), ecdsa_sig.get(), user_id,
                          user_id_len, msg, msg_len);
  SPDLOG_DEBUG("sm2_do_verify ret=%d", ret);
  SPDLOG_DEBUG("msg_len=%d sig_len=%d raw_key_len=%d", msg_len, sig_len,
             raw_key_len);
  return ret;
}

static int get_cer_into_der(uint8_t* pem_cert,
                                     int cert_len,
                                     std::string* der_cert) {
  ...
  std::vector<unsigned char> der_cert_vector = cppcodec::base64_rfc4648::decode(base64_cert);

  // Convert the vector to a string and assign it to the der_cert
  *der_cert = std::string(der_cert_vector.begin(), der_cert_vector.end());
  return 0;
}

static int get_pubkey_from_cert(uint8_t* cert,
                                         int cert_len,
                                         sm2_pub_key_t* pubkey) {
  std::string der_cert;
  YACL_ENFORCE_EQ(get_cer_into_der(cert, cert_len, &der_cert),0, "get_cer_into_der failed");
  ...
  if (cert_x509.get() == nullptr) {
    SPDLOG_ERROR("Fail to load ca certificate");
    return -1;
  }
  UniquePkey pkey(X509_get_pubkey(cert_x509.get()), EVP_PKEY_free);
  if (pkey.get() == nullptr) {
    SPDLOG_ERROR("Fail to get public key from ca certificate");
    return -1;
  }
  UniqueEcKey eckey(EVP_PKEY_get1_EC_KEY(pkey.get()), EC_KEY_free);
  if (eckey.get() == nullptr) {
    SPDLOG_ERROR("Fail to get the EC key from public key");
    return -1;
  }
  ...
  if (keylen != EC_POINT_point2oct(group, point, POINT_CONVERSION_UNCOMPRESSED,
    ...
    SPDLOG_ERROR("Fail to get the raw pub key");
    return -1;
  }
...
static int verify_der_cert(uint8_t* der_cert,
                           ...
                           int ca_cert_len) {
  sm2_pub_key_t pubkey;
  if (get_pubkey_from_cert(ca_cert, ca_cert_len, &pubkey) != 0) {
    return -1;
  }
  ...
  if (ret <= 0 || tbs_cert_len <= 0 || cert_sig_len <= 0) {
    SPDLOG_ERROR("Fail to parse the der certificate");
    return -1;
  }
...
static int check_cfca_cert_issuer(uint8_t* der_cert, int cert_len) {
  ...
  if (!issuer) {
    SPDLOG_ERROR("Fail to get issuer");
    return -1;
  }

  char* pcn = strstr(issuer, cnPrefix);
  if (!pcn) {
    SPDLOG_ERROR("pcn info error");
    return -1;
  }
...
bool verify_peer_cert(uint8_t* der_cert, int der_cert_len) {
  ...
  if (!der_cert || !der_cert_len) {
    SPDLOG_ERROR("Invalid certificate to be verified");
    return false;
  }

  bool ret = false;
  if (check_cfca_cert_issuer(der_cert, der_cert_len) == 0) {
  ...
  if (ret) {
    SPDLOG_DEBUG("Verify peer certificate successfully");
  } else {
    SPDLOG_ERROR("Fail to verify peer certificate");
  }
